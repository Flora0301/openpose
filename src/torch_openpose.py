...
class torch_openpose(object):
    ...
    def __call__(self, oriImg):
        ...
        all_peaks = []  # 所有关键点信息，总共25个关键点，每个元素里边有每个人关键点坐标
        peak_counter = 0
        ...
        # print("all_peak_len: " + str(len(all_peaks)))
            ...
            if (nA != 0 and nB != 0):
                ...
                for i in range(nA):
                    for j in range(nB):
                        ...
                        norm = math.sqrt(vec[0] * vec[0] + vec[1] * vec[1]) + math.exp(0.00000000000001)
                        ...
                        score_with_dist_prior = sum(score_midpts) / len(score_midpts) + min(
                            0.5 * oriImg.shape[0] / (norm + math.exp(0.00000000000001)) - 1, 0)
        ...
        for i in range(len(subset)):
            if subset[i][-1] < 4 or subset[i][-2] / subset[i][-1] < 0.4:
                deleteIdx.append(i)
        subset = np.delete(subset, deleteIdx, axis=0)  # 识别到的对象
        # print(subset)
        
        poses = []
        heels = []
        # per记录每个人的关键点标号
        # print("subste_len: " + str(len(subset)))
        ...
        for per in subset:
            ...
            for po in per[:-2]:
                if po >= 0:
                    ...
                    joint = [0.,0.,0.]
                pose.append(joint)
                # heel.append(joint[21])
                # heel.append(joint[24])
                # print("joint: " + str(joint))
            # print("-------------------------------------------------")
            poses.append(pose)
            heels.append(pose[21])
            heels.append(pose[24])
        ...
        for i in range(end):
            if i % 2 == 1:
                ...
                if (y >= start) & (x > 50):
                    start = y
                    target = i
        heel.append(heels[target-1])
        heel.append(heels[target])

        # 获取目标人物的关键点
        position = []
        p = []
        target_i = (target-1) / 2
        # print("target_i:" + str(target_i))
        target_indx = subset[int(target_i)]
        for j in target_indx[:-2]:
            if j >= 0:
                joint = list(candidate[int(j)][:3])
            else:
                joint = [0., 0., 0.]
            position.append(joint)
        p.append(position)

        # print("position: " + str(position))
        # print("pose:" + str(poses))

        return poses, heels, heel, p
